...
public class CloudEventBuilderTest {
    ...
    void testCopyWithBuilder(CloudEventImpl event) {
        assertThat(BaseCloudEventBuilder.buildV03(event).build()).isEqualTo(event);
    }

    @ParameterizedTest()
    @MethodSource("io.cloudevents.test.Data#v03Events")
    void testToV1(CloudEventImpl event) {
        CloudEventImpl eventV1 = BaseCloudEventBuilder.buildV1(event).build();
