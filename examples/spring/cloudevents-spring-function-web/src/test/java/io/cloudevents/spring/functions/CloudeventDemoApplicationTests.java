...
//	        HttpHeaders headers = this.buildHeaders(MediaType.valueOf(CloudEventAttributeUtils.APPLICATION_CLOUDEVENTS_VALUE + "+json"));
            ...
            // Uncomment this and comment the next two assertion if CloudEventAttributesProvider is enabled in CloudeventDemoApplication
//	        assertThat(response.getHeaders().getFirst("ce-" + CloudEventAttributeUtils.SOURCE)).isEqualTo("https://interface21.com/");
//	        assertThat(response.getHeaders().getFirst("ce-" + CloudEventAttributeUtils.TYPE)).isEqualTo("com.interface21");

            assertThat(response.getHeaders().getFirst("ce-" + CloudEventAttributeUtils.SOURCE))
                    .isEqualTo("http://spring.io/application-application");
            assertThat(response.getHeaders().getFirst("ce-" + CloudEventAttributeUtils.TYPE))
...
//            assertThat(response.getHeaders().getFirst("ce-" + CloudEventAttributeUtils.TYPE)).isEqualTo("com.interface21");
	...
	private HttpHeaders buildHeaders(MediaType contentType) {
		...
		headers.set(CloudEventAttributeUtils.HTTP_ATTR_PREFIX + CloudEventAttributeUtils.ID, UUID.randomUUID().toString());
		headers.set(CloudEventAttributeUtils.HTTP_ATTR_PREFIX + CloudEventAttributeUtils.SOURCE, "https://spring.io/");
		headers.set(CloudEventAttributeUtils.HTTP_ATTR_PREFIX + CloudEventAttributeUtils.SPECVERSION, "1.0");
		headers.set(CloudEventAttributeUtils.HTTP_ATTR_PREFIX + CloudEventAttributeUtils.TYPE, "org.springframework");
		return headers;
	}
}
